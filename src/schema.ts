...
export class Factory<T extends { [name: string]: Structure<T>; }> {
    ...
    private checkType(
        ...
        if (this.isRefType(t)) {
            ...
            for (const candidate of allowed) {
                if (this.check(rid, root, candidate, map)) return true;
            }
            return false;
        }
...
type Ref<TDefs extends { [name: string]: Structure<TDefs>; }, K extends keyof TDefs> = { ref: string, getValue(): Interfaces<TDefs>[K] }
